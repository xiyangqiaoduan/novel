    ...
    public void main() {
        ...
        for (final String key : TOP_NAME_MAP.keySet()) {
            try {
                ...
                Element element = document.select("div.page a").last();
                ...
                for (int i = Integer.parseInt(last_num); i > 0; i--) {
                    ...
                    System.out.println("当前页：" + pageUrl);
                        ...
                        taskExecutor.execute(new Runnable() {
                            public void run() {
                                ...
                                try {
                                    ...
                                    System.out.println("文章地址：" + article_url);
                                    ...
                                    System.out.println("封面地址：" + img_url);
                                    String title = contentDocument.select("div div.info-right div.desc h1").text();
                                    System.out.println("小说名称：" + title);
                                    String author = contentDocument.select("div div.info-right div.descTip p span").get(2).text();
                                    System.out.println("小说作者：" + author);
                                    String intro = contentDocument.select("div div.info-right div.descInfo p").text();
                                    System.out.println("小说简介：" + intro);
                                    ...
                                    for (Element chapterUrlElement : chapterUrlElements) {
                                        ...
                                        System.out.println("章节地址：" + chapterUrl);
                                        if (!StringUtils.contains(chapterUrl, URL)) {
                                            chapterUrl = URL + chapterUrl;
                                        }
                                        Document chapterContentDocument = Jsoup.connect(chapterUrl).userAgent("Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.86 Safari/537.36").get();
                                        String content = chapterContentDocument.select("div#mycontent").text();
                                        System.out.println("章节的内容：" + content);
