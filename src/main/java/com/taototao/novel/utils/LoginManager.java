...
public class LoginManager {
    ...
    public static void setReferer() {
        ...
        HttpServletRequest request =  ((ServletRequestAttributes)RequestContextHolder.getRequestAttributes()).getRequest();
        String refererPath= request.getHeader(REFERER_KEY);

        // 如果URL里没有本站域名的话，设置本站域名为默认值
        if (!(StringUtils.contains(refererPath, TaoToTaoConstants.taoToTaoConf.getString(TaoToTaoConfig.URI)) && StringUtils
                .contains(refererPath, TaoToTaoConstants.taoToTaoConf.getString(TaoToTaoConfig.URI)))) {
            refererPath = TaoToTaoConstants.taoToTaoConf.getString(TaoToTaoConfig.URI);
        }
    ...
    public static final String getAndCleanReferer() {
        ...
        if (session == null || StringUtils.isEmpty(session.getAttribute(REFERER_KEY).toString())) {
            return TaoToTaoConstants.taoToTaoConf.getString(TaoToTaoConfig.URI);
        }
    ...
    public static final User getLoginUser() {
        ...
        if (Utils.isDefined(session)) {
            return (User) session.getAttribute(TaoToTaoConstants.LOGINUSER);
        } else {
            return null;
        }
    }
     ...
     *            用户信息
     */
    public static final void doLogin(final User user) {
        HttpSession session = getSession(true);
        session.setAttribute(TaoToTaoConstants.LOGINUSER, user);
    }
    ...
    public static final void doLogout() {
        HttpSession session = getSession(true);
        session.setAttribute(TaoToTaoConstants.LOGINUSER, null);
    }
}
