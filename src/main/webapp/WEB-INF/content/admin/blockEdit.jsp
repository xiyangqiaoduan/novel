...
<body>
<form action="${pageContext.request.contextPath}/admin/systemblock/edit" method="post"></form>
...
<table class="yidu-table" align="center">
    ...
    <tr>
        <td>区块ID</td>
        <td>
            <input type="text" name="blockid" id="blockid" maxlength="32" class="tb-rounded" value="${block.blockid}"/>
        </td>
    </tr>
    <tr>
        <td>区块名</td>
    ...
    <tr>
        ...
        <td>
            <select id="type" name="type">
                <option value="10" <c:if test="${block.type == 10}">selected</c:if> >标准小说列表(各种排行榜单)</option>
                <option value="40" <c:if test="${block.type == 40}">selected</c:if>>随机小说列表(为了性能，暂不支持类型)</option>
                <option value="50" <c:if test="${block.type == 50}">selected</c:if>>动态推荐小说列表(同分类当前小说的后几本，只用于简介，章节列表，阅读页)</option>
                ...
                <option value="60" <c:if test="${block.type == 60}">selected</c:if>>和显示名字相似的小说(只用于简介，章节列表，阅读页)</option>
    ...
    <tr>
        ...
        <td>
            <select id="sortcol" name="sortcol">
                ...
                <option value="lastupdate"  <c:if test="${block.sortcol eq 'lastupdate'}">selected</c:if>>最后更新时间</option>
                ...
                <option value="monthvisit" <c:if test="${block.sortcol eq 'monthvisit'}">selected</c:if>>月排行</option>
    ...
    <tr>
        <td>升序</td>
        <td>
            <input type="radio" name="isasc" value="1" <c:if test="${block.isasc ==true}">checked</c:if> />是
    ...
    <tr>
        <td colspan="2" style="text-align: center; padding: 2px">
            <input type="submit" value="返回" class="submitButton" theme="simple" onclick="goback();"/>

            <c:choose>
                <c:when test="${block.blockno==0}">
